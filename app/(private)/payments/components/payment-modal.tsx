...
export function PaymentModal({ payment, defaultTutor, onClose, onSuccess }: PaymentModalProps) {
  ...
  return (
    <Modal
      title={isEditing ? 'Editar Cobran̤a' : 'Nova Cobran̤a'}
      description={isEditing ? 'Atualize os itens e dados da cobran̤a' : 'Adicione produtos e servi̤os para gerar a cobran̤a'}
      onClose={onClose}
      maxWidth="lg"
    >
      <form onSubmit={(e) => { void handleSubmit(e); }} className="flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <Button type="submit" disabled={saving} className="bg-teal-600 hover:bg-teal-700 text-white border-teal-600">
            {saving ? <Loader2 size={16} className="animate-spin" /> : isEditing ? 'Salvar altera̵̤es' : `Registrar ${total > 0 ? fmtCurrency(total) : ''}`}
          </Button>
        </div>
      </form>
    </Modal>

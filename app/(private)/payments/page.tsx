  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 w-full">
      <div className="w-full px-4 sm:px-6 lg:px-8 py-2">
        ...
        <SectionCard
          title="CobranÃ§as"
          subtitle={meta ? `${meta.total_elements} cobranÃ§a${meta.total_elements !== 1 ? 's' : ''} no total` : 'Carregando...'}
          headerAction={
            <Button
        ...
        >
          ...
          <DataTable
            headers={['Itens', 'Valor', 'Status', 'Vencimento', 'Pago em', 'AÃ§Ãµes']}
              ...
              <tr>
                <td colSpan={6} className="p-8 text-center">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm">
                    {statusFilter || tutorFilter ? 'Nenhuma cobranÃ§a encontrada para os filtros selecionados.' : 'Nenhuma cobranÃ§a cadastrada ainda.'}
                  </p>
                </td>
              </tr>
            ) : (
              payments.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="p-4 text-slate-600 dark:text-slate-300 text-sm">
                      {p.paid_at ? fmtDate(p.paid_at) : <span className="text-slate-400 dark:text-slate-500">â€”</span>}
                    </td>
                    <td className="p-4">
      ...
      {confirmDelete && (
        <ConfirmModal
          ...
          description={`Esta cobranÃ§a de ${fmtAmount(confirmDelete.amount)} serÃ¡ removida permanentemente.`}
